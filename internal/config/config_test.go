...
func TestValidation(t *testing.T) {
	cases := []struct {
		...
		{"requires kafka brokers when only the template topic is set",
			append(baseline(), "NFC_KAFKA_TEMPLATE_TOPIC=templates"), "NFC_KAFKA_BROKERS", "must be set"},
		{"rejects a template topic equal to the flow topic",
			append(baseline(), "NFC_KAFKA_BROKERS=127.0.0.1:19092", "NFC_KAFKA_TOPIC=flows", "NFC_KAFKA_TEMPLATE_TOPIC=flows"),
			"NFC_KAFKA_TEMPLATE_TOPIC", "must differ"},
